...
For the deterministic character layer, `--clean` emits the scrubbed text instead of JSON - it strips the invisible and non-printing codepoints, normalizes non-standard spaces to `U+0020`, folds mixed-script homoglyph letters back to ASCII (#66), and trims stray whitespace. Visible content is left alone, including a Markdown hard break, which is two spaces before a newline and survives the trim. Only ever redirect this into a **new plain-text file**: given a `.docx`, `.epub`, `.odt` or `.ipynb` it prints the extracted text rather than a rebuilt document, so writing it back over the original would replace the document with loose text.
